...
## Overview
This migration creates the internal messaging/chat system for the TaskFlow Hub application.
...
CREATE INDEX idx_messages_chat_group ON messages(chat_group);
CREATE INDEX idx_messages_created_at ON messages(created_at DESC);
CREATE INDEX idx_messages_unread ON messages(recipient_id, is_read) WHERE recipient_id IS NOT NULL;
...
CREATE POLICY "Users can view received individual messages" ON messages
  FOR SELECT
  USING (
...
CREATE POLICY "Users can view section messages" ON messages
  FOR SELECT
  USING (
...
CREATE POLICY "Users can view level messages" ON messages
  FOR SELECT
  USING (
    chat_type = 'level' AND
...
AS $$
  ...
  WHERE (
    ...
    (chat_type = 'section' AND is_read = false AND chat_group IN (
      SELECT unnest(sections) FROM profiles WHERE id = user_id
    )) OR
    (chat_type = 'level' AND is_read = false AND chat_group = (
